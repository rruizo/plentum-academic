-- PASO 1 CORREGIDO v2: Crear las nuevas tablas según especificación del sistema de Evaluación Cognitiva Integral
...
-- 5. Migrar datos existentes de preguntas_cognitivas a preguntas_cognitivas_banco (CORREGIDO v2)
...
SELECT 
    ...
    opciones_respuestas,
    CASE 
        WHEN respuesta_correcta_id ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' 
        THEN respuesta_correcta_id::UUID
        ELSE NULL 
    END, -- CORREGIDO: Cast seguro de text a UUID
