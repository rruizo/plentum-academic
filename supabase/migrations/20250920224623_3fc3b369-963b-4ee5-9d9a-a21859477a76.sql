-- Fix the HTP assignment access validation function to properly match UUID
-- Drop and recreate the function with corrected UUID matching logic
DROP FUNCTION IF EXISTS public.validate_htp_assignment_access(text);

-- Create the security definer function with proper UUID matching
...
AS $$
  ...
  WHERE ha.access_link LIKE '%/' || p_access_link
