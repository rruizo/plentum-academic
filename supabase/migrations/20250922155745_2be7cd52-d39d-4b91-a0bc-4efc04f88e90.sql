...
-- 5. Add logging for sensitive exam access
CREATE OR REPLACE FUNCTION log_exam_access()
RETURNS TRIGGER AS $$
BEGIN
  -- Log exam access for security monitoring
...
-- Create trigger for exam access logging
DROP TRIGGER IF EXISTS exam_access_log_trigger ON public.exams;
CREATE TRIGGER exam_access_log_trigger
  AFTER SELECT ON public.exams
  FOR EACH ROW
  EXECUTE FUNCTION log_exam_access();